    ...
    <div className="my-8 p-6 bg-white rounded-xl border border-slate-200">
        ...
        {currentScenes.map((scene, index) => (
          <div key={scene.num} className="flex items-center flex-1">
            <div className="flex flex-col items-center flex-1">
              <div className={`w-8 h-8 rounded-full ${config.accentColor} text-white flex items-center justify-center text-sm font-bold mb-1 transition-colors duration-300`}>
                {scene.num}
              </div>
            ...
            {index < currentScenes.length - 1 && (
              <div className={`h-0.5 flex-1 mx-1 transition-colors duration-300 ${scenario === 'none' ? 'bg-slate-300' : scenario === 'jem' ? 'bg-amber-200' : 'bg-red-200'}`} />
            )}
          </div>
        ))}
      </div>
      <div className="mt-4 text-center">
        <div className="inline-flex items-center gap-2 text-xs text-slate-500">
          <div className={`w-2 h-2 rounded-full ${config.accentColor} transition-colors duration-300`}></div>
